...
class Server(BasicServer):
        ...
        probability = (
            ...
            * (
                math.exp(
                    -0.5
                    * (
                        (math.log(self.current_round) - self.option["u"])
                        / self.option["o"]
                    )
                    ** 2
                )
    ...
    def modify_ratio(
        ...
        print(
            f"Update ratio round {self.current_round}: {self.option['ratio']} >>>>  {sample_ratio}"
        )
    ...
    def iterate(self):
        ...
        dys, dcs = received_information['dy'], received_information['dc']
...
class Client(BasicClient):
    ...
    def unpack_model_range(self, svr_pkg):
        return svr_pkg["model"], svr_pkg["score_range"], svr_pkg["cg"]
    ...
    def reply_score(self, svr_pkg):
        model, score_range, c_g = self.unpack_model_range(svr_pkg)
        self.c_g = c_g
    ...
    def reply(self, svr_pkg):
        ...
        dy, dc = self.train(self.model, copy.deepcopy(self.c_g))
        cpkg = self.pack(dy, dc, len(selected_idx))
        return cpkg

    def pack(self, dy, dc, n_trained_samples):
        return {"dy": dy, 
                "dc": dc, 
    ...
    def train(self, model, cg):
        ...
        for epoch in range(self.epochs):
            ...
            for data, labels, idxs in self.data_loader:
                ...
                loss.backward()
                for pm, pcg, pc in zip(
                    model.parameters(), cg.parameters(), self.c.parameters()
                ):
