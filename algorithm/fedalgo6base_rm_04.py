...
class Server(BasicServer):
    ...
    def cal_threshold(self, aggregated_histogram):
        # breakpoint()
        if self.current_round > self.option["warmup_rounds"]:
...
class Client(BasicClient):
    def __init__(self, option, name="", train_data=None, valid_data=None, device="cpu"):
        ...
        self.score_cached_list = []
        
    def calculate_importance(self, model):
        criteria = nn.CrossEntropyLoss()
        _, score_list_on_cl = utils.fmodule.Sampler.cal_cof(
            self.train_data, model, criteria, self.device
        )
        self.score_cached_list.append(score_list_on_cl)
        sc = np.array(self.score_cached_list)[-10:].max(0)
        
        self.score_cached = list(sc)
