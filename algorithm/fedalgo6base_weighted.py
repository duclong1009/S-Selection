...
def cal_n_samples_per_label(dataset, index_list):
    count_label_dict = {}
    for i in index_list:
        # 
        x,y = dataset[i][0], dataset[i][1]
        if y in count_label_dict.keys():
...
class Client(BasicClient):
    def __init__(self, option, name="", train_data=None, valid_data=None, device="cpu"):
        ...
        self.raw_n_labels = cal_n_samples_per_label(train_data,range(len(train_data)))
    ...
    def reply(self, svr_pkg):
        ...
        stat_current_round = cal_n_samples_per_label(self.train_data,selected_idx)
        # self.raw_n_labels
        weight_list = []
        for i in range(10):
            if stat_current_round[i] ==0 :
                ...
                weight_list.append(1.0*self.raw_n_labels[i]/ stat_current_round[i])
        self.weighted = torch.tensor(weight_list)
        # 
    ...
    def train(self, model):
        ...
        """
        # self.criterion = 
        self.criterion = torch.nn.CrossEntropyLoss(weight = self.weighted.to(self.device))
