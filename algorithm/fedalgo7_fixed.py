...
class FuzzyLogicBase:
    def __init__(self, config) -> None:
        ...
        self.history_cached_config = self.config["noisy_rate"]
...
class Server(BasicServer):
    ...
    def cal_noisy_rate(self):
        total_noisy = 0
        total = 0 
        for client in self.selected_clients:
            total_noisy += self.local_data_vols[client] * self.noisy_rate[client]
            total += self.local_data_vols[client]
        return total_noisy * 1.0 / total
        ...
        removed_percentage = self.fuzzy_algo.run(
            {
                ...
                "history_cached": self.cal_noisy_rate(),
