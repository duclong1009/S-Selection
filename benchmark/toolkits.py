...
class BasicTaskGen:
    ...
    def __init__(
        self, benchmark, dist_id, skewness, rawdata_path, local_hld_rate=0.2, seed=0
    ):
        """
...
class DefaultTaskGen(BasicTaskGen):
    def __init__(
        ...
        super(DefaultTaskGen, self).__init__(
            benchmark, dist_id, skewness, rawdata_path, local_hld_rate, seed
        )
        self.minvol = minvol
        self.num_classes = -1
    ...
    def run(self):
        ...
        self.train_cidxs, self.valid_cidxs = self.local_holdout(
            local_datas, shuffle=True
        )
        print("Done.")
    ...
    def partition(self):
        ...
        if self.dist_id == 0:
            ...
            proportions = [
                np.random.dirichlet(alpha * p) for _ in range(self.num_clients)
            ]
            ...
            while True:
                ...
                exclude_norms = []
                for cid in range(self.num_clients):
                    mean_excid = (mean_prop * self.num_clients - proportions[cid]) / (
                        self.num_clients - 1
                    )
                ...
                for cid in range(self.num_clients):
                    ...
                    mean_alter_cid = (
                        mean_prop
                        - proportions[excid] / self.num_clients
                        + sup_prop[cid] / self.num_clients
                    )
            ...
            for lb in lb_counter.keys():
                ...
                lb_proportion = (np.cumsum(lb_proportion) * len(lb_idxs)).astype(int)[
                    :-1
                ]
                lb_datas = np.split(lb_idxs, lb_proportion)
                self.dirichlet_dist.append([len(lb_data) for lb_data in lb_datas])
                local_datas = [
                    local_data + lb_data.tolist()
                    for local_data, lb_data in zip(local_datas, lb_datas)
                ]
            ...
            sigma = 1
            samples_per_client = np.random.lognormal(
                mu, sigma, (self.num_clients)
            ).astype(int)
            ...
            while crt_data_size != total_data_size:
                if crt_data_size - total_data_size >= thresold:
                    ...
                    new_size_id = np.argmin(
                        [
                            np.abs(crt_data_size - samples_per_client[maxid] + s)
                            for s in samples
                        ]
                    )
                    ...
                    new_size_id = np.argmin(
                        [
                            np.abs(crt_data_size - samples_per_client[minid] + s)
                            for s in samples
                        ]
                    )
            ...
            sorted_cid_map = {
                k: i
                for k, i in zip(
                    np.argsort(samples_per_client), [_ for _ in range(self.num_clients)]
                )
            }
            ...
            while True:
                ...
                # generate dirichlet distribution till ||E(proportion) - P(D)||<=1e-5*self.num_classes
                mean_prop = np.sum(
                    [pi * di for pi, di in zip(proportions, samples_per_client)], axis=0
                )
            ...
            for lb in lb_counter.keys():
                lb_idxs = lb_dict[lb]
                lb_proportion = np.array(
                    [pi[lb] * si for pi, si in zip(proportions, samples_per_client)]
                )
                lb_proportion = lb_proportion / lb_proportion.sum()
                lb_proportion = (np.cumsum(lb_proportion) * len(lb_idxs)).astype(int)[
                    :-1
    ...
    def visualize_by_class(self, train_cidxs):
        ...
        if hasattr(self, "dirichlet_dist"):
            ...
            row_map = {
                k: i
                for k, i in zip(
                    np.argsort(data_columns), [_ for _ in range(self.num_clients)]
                )
            }
            for cid, cprop in enumerate(client_dist):
                ...
                for lbi in range(len(cprop)):
                    plt.fill_between(
                        [offset, offset + cprop[lbi]],
                        y_bottom,
                        y_top,
                        facecolor=colors[lbi],
                    )
...
class ClassificationCalculator(BasicTaskCalculator):
    ...
    def test(self, model, dataset, batch_size=64, num_workers=0):
        ...
        for batch_id, batch_data in enumerate(data_loader):
            ...
            list_y_pred += y_pred.squeeze().tolist()
            list_y_gr += batch_data[-1].tolist()
            num_correct += correct.item()
            total_loss += batch_mean_loss * len(batch_data[-1])
        from sklearn.metrics import confusion_matrix
        conf_matrix = confusion_matrix(list_y_gr, list_y_pred)
        return {
            "accuracy": 1.0 * num_correct / len(dataset),
...
class XYTaskPipe(BasicTaskPipe):
    ...
    def save_task(cls, generator):
        ...
        for cid in range(len(generator.cnames)):
            feddata[generator.cnames[cid]] = {
                "dtrain": {
                    "x": [
                        generator.train_data["x"][did]
                        for did in generator.train_cidxs[cid]
                    ],
                    "y": [
                        generator.train_data["y"][did]
                        for did in generator.train_cidxs[cid]
                    ],
                },
                "dvalid": {
                    "x": [
                        generator.train_data["x"][did]
                        for did in generator.valid_cidxs[cid]
                    ],
                    "y": [
                        generator.train_data["y"][did]
                        for did in generator.valid_cidxs[cid]
    ...
    def load_task(cls, task_path):
        ...
        for name in feddata["client_names"]:
            train_x, train_y = (
                feddata[name]["dtrain"]["x"],
                feddata[name]["dtrain"]["y"],
            )
            valid_x, valid_y = (
                feddata[name]["dvalid"]["x"],
                feddata[name]["dvalid"]["y"],
            )
...
class IDXTaskPipe(BasicTaskPipe):
    ...
    def load_task(cls, task_path):
        ...
        origin_train_data = cls.args_to_dataset(
            origin_class, feddata["datasrc"]["train_args"]
        )
        origin_test_data = cls.args_to_dataset(
            origin_class, feddata["datasrc"]["test_args"]
        )
        test_data = cls.TaskDataset(
            origin_test_data, [_ for _ in range(len(origin_test_data))]
        )
        train_datas = []
        valid_datas = []
        for name in feddata["client_names"]:
    ...
    def save_task(cls, generator):
        ...
        for cid in range(generator.num_clients):
            feddata[generator.cnames[cid]] = {
                "dtrain": {
                    "x": [
                        generator.train_data[did] for did in generator.train_cidxs[cid]
                    ],
                },
                "dvalid": {
                    "x": [
                        generator.train_data[did] for did in generator.valid_cidxs[cid]
                    ],
                },
            }
        with open(os.path.join(generator.taskpath, "data.json"), "w") as outf:
...
class PillImageFolder(ImageFolder):
