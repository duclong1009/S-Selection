import sys
sys.path.append('/home/ace15032jo/Projects/repo2/')
import wandb
import torch
...
if __name__ == "__main__":
    ...
    if args.log_wandb:
        wandb.init(
            ...
            entity="aiotlab",
            group=f"Centrailize_9",
            name=args.session_name,
            config=args,
        )
    with open("centrailize/Without_rm_eval_loginterval_3/cluster_id_9.json","r") as f:
        rm_dict = json.load(f)
    ...
    with open(args.idx_path,"r") as f:
        import json
        data_idx = json.load(f)["blurry_id"]
    single_set = {"0":[]}
    ...
    #     model.train()
    #     training_loss = 0
    #     for data, labels, idxs in tqdm(train_dataloader):
    ...
    model.load_state_dict(torch.load("/home/ace15032jo/Projects/repo2/resnet18.pt"))
    # 
    ...
    import os
    # breakpoint)
    if not os.path.exists(f"using_pretrained/{args.session_name}"):
        os.makedirs(f"using_pretrained/{args.session_name}")
    with open(f"using_pretrained/{args.session_name}/conf_arr_.npy","wb") as f:
        np.save(f, conf_array)

    with open(f"using_pretrained/{args.session_name}/result_.json","w") as f:
        json.dump(saved_dict,f)
    torch.save(model.state_dict(),f"using_pretrained/{args.session_name}/model_.pt")
