...
class Logger(logging.Logger):
    ...
    def __init__(self, meta, *args, **kwargs):
        ...
        if self.meta["log_file"]:
            ...
            self.log_path = os.path.join(
                log_dir, self.get_time_string() + self.get_output_name(".log")
            )
        ...
        for key, value in self.output.items():
            if not self.check_is_jsonable(value):
                try:
                    ...
                    self.warning(
                        "flw.logger.output['{}'] is not jsonable, and is automatically converted to string.".format(
                            key
                        )
                    )
                except:
                    del self.output[key]
                    self.warning(
                        "Automatically remove flw.logger.output['{}'] from logger, because it is not jsonable and is failed to convert into string. ".format(
                            key
                        )
                    )
        return
    ...
    def write_var_into_output(self, var_name=None, var_value=None):
        ...
        if var_name == None:
            raise RuntimeError("Missing the name of the variable to be logged.")
    ...
    def get_output_name(self, suffix=".json"):
        ...
        if hasattr(self, "server"):
            ...
            if self.meta["algo_para"] is not None:
                header = (
                    header
                    + "algopara_"
                    + "|".join([str(p) for p in self.meta["algo_para"]])
                )

        output_name = header + "M{}_R{}_B{}_".format(
            self.meta["model"], self.meta["num_rounds"], self.meta["batch_size"]
        )
        ...
        output_name = (
            output_name
            + "LR{:.4f}_P{:.2f}_S{}_LD{:.3f}_WD{:.3f}".format(
    ...
    def early_stop(self):
        ...
        if self._es_best_score is None:
            ...
            if self._es_counter >= self._es_patience:
                self.info(
                    "Early stopping after training for {} rounds.".format(
    ...
    def log_once(self, *args, **kwargs):
        ...
        for met_name, met_val in test_metric.items():
            if not (met_name == "confusion_matrix" or met_name == 'inference_arr'): 
                # 
        ...
        utils.fmodule.LOG_WANDB["Confusion_matrix"] = test_metric["confusion_matrix"].tolist()
        # 
